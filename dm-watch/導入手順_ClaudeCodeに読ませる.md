...
6. 「**2. アクセストークンを生成する**」→「**アカウントを追加**」→ モーダル「Instagramアカウントを追加」の「**次へ**」
...
7. トークンが表示されたら：**読み上げない・書き出さない**。担当者に「表示されたトークンを、パスワード管理ツール等にコピーして保存してください。あとで Claude の環境に貼り付けます」と伝える
8. 発行日を控えてもらう（60日後が期限）
9. 🔵 「**3. Webhooksを設定する**」は**設定しない**。このキットはポーリング方式なので不要
